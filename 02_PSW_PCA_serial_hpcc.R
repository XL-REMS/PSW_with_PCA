...
###  Author: Seungman Kim, Yifang Zeng, Xinyang Li ###
...
##Log
######################################################################################
# 03-28-2020: runing on testset
# 04-03-2020: finsih running on local full dataset
# 04-12-2020: finish editing for serial computation on hpcc
# *parallel version will be a sperate file
...
for (wrapitr in 1:27){
  ...
  #############################################
  for (i in 1:length(file_list)) { # start of i loop
    fill_active_list()
    print(i)
    # print(paste0("group:",wrapitr,", ",i, ": act_list"))
  } # end of i loop
  ...
  colnames(dat_ana) <- c("XX", "TY", "TS", "IT", "Nops_b","Nops_r2",
                         ...
                         "Odds_low","Odds_up","Iptw_b","Iptw_r2",
                         ...
                         "Odds_loW_PC2", "Odds_up_PC2", "Iptw_b_PC2",
  ...
  #Memory management
  rm(dat_ana, act_list) # clear dat_and, act_list objects.
